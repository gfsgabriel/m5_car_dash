...
// CORREÇÃO: Enum completo restaurado com todas as telas
enum ModosTela { 
  TELA_HUD_PRINCIPAL, 
  TELA_MENU_CONFIG, 
  TELA_SUB_PAREAR, 
...
static float ANGULO_MIN = -60.0; static float ANGULO_MAX = 60.0;
static const int pivotX_G = 160; static const int pivotY_G = 180; 
static const int pivotX_R = 80;  static const int pivotY_R = 90;  

int barraLarguraBloco = 4; int barraLarguraEspaco = 1; int alturaBarraRPM = 32; int inicioCorteY = 18; 
int corteVelocidadeX = 110; int corteVelocidadeLarg = 100; int corteRpmDigitalX = 230; int corteRpmDigitalLarg = 85; 

// Declaração externa da função do obd2_manager para alterar a litragem
...
int obterModoTelaAtual() { return (int)modoAtual; }
...
static void desenharIconeBateria(int x, int y) {
  canvasVirtual.drawRect(x, y + 2, 14, 9, WHITE); canvasVirtual.fillRect(x + 2, y, 3, 2, WHITE);  
  canvasVirtual.fillRect(x + 9, y, 3, 2, WHITE);  canvasVirtual.drawFastHLine(x + 2, y + 6, 3, WHITE); 
  canvasVirtual.drawRect(x + 9, y + 5, 3, 3, WHITE);  
}
static void desenharIconeCoolant(int x, int y) {
  canvasVirtual.drawRect(x + 5, y, 4, 10, WHITE); canvasVirtual.fillCircle(x + 7, y + 10, 4, WHITE);
  canvasVirtual.drawFastHLine(x, y + 13, 14, CYAN); canvasVirtual.drawFastHLine(x + 2, y + 15, 10, CYAN);
}
static void desenharIconeIntake(int x, int y) {
  canvasVirtual.drawRect(x, y + 2, 14, 8, WHITE); canvasVirtual.fillRect(x + 5, y, 4, 12, BLUE); 
}
static void desenharIconeTPS(int centroX, int centroY, float tpsPercent) {
  int raioBola = 6; int alturaTracoMax = 24; canvasVirtual.drawCircle(centroX, centroY, raioBola, WHITE);
  ...
  canvasVirtual.setFont(&fonts::Font0); canvasVirtual.setTextSize(1.0); canvasVirtual.setTextColor(CINZA_CLARO);
  canvasVirtual.setTextDatum(textdatum_t::top_center); canvasVirtual.drawString(String((int)tpsPercent) + "% tps", centroX, centroY + 14);
...
void renderizarHUDPrincipal() {
  ...
  canvasVirtual.setFont(&fonts::Font4); canvasVirtual.setTextSize(0.75); canvasVirtual.setTextColor(WHITE);
  ...
  canvasVirtual.setFont(&fonts::Font7); canvasVirtual.setTextSize(0.9);
  ...
  canvasVirtual.setTextColor((c == 0) ? BLACK : WHITE); canvasVirtual.setCursor(startX, posY); canvasVirtual.printf("%d", c);
  canvasVirtual.setTextColor((c == 0 && d == 0) ? BLACK : WHITE); canvasVirtual.setCursor(startX + largCaractere, posY); canvasVirtual.printf("%d", d);
  canvasVirtual.setTextColor(WHITE); canvasVirtual.setCursor(startX + (largCaractere * 2), posY); canvasVirtual.printf("%d", u);
  
  canvasVirtual.setFont(&fonts::Font0); canvasVirtual.setTextSize(1.5); canvasVirtual.setCursor(192, 70); canvasVirtual.print("km/h");

  int offsetHUD_Y = 42;
  desenharIconeBateria(10, offsetHUD_Y); canvasVirtual.setCursor(30, offsetHUD_Y); canvasVirtual.printf("%.1fV", telemetria.bateria);
  desenharIconeCoolant(10, offsetHUD_Y + 18); canvasVirtual.setCursor(30, offsetHUD_Y + 18); canvasVirtual.printf("%.0f C", telemetria.tempCoolant);
  desenharIconeIntake(10, offsetHUD_Y + 36); canvasVirtual.setCursor(30, offsetHUD_Y + 36); canvasVirtual.printf("%.0f C", telemetria.tempIntake);

  canvasVirtual.setFont(&fonts::Font2); canvasVirtual.setTextSize(1.0);
  canvasVirtual.setTextDatum(textdatum_t::top_left); canvasVirtual.setTextColor(CYAN);
  ...
  canvasVirtual.setTextDatum(textdatum_t::top_right); canvasVirtual.setTextColor(CYAN);
  ...
  if (telemetria.boost > 15.0) {
    canvasVirtual.fillCircle(15, 132, 5, RED); canvasVirtual.drawCircle(15, 132, 7, WHITE);
  }
  ...
  if (telemetria.consumo_ml_min > 400.0) {
    canvasVirtual.fillCircle(300, 132, 5, RED); canvasVirtual.drawCircle(300, 132, 7, WHITE);
    int animOffset = (millis() / 15) % 30;
    canvasVirtual.setFont(&fonts::Font2); canvasVirtual.setTextColor(YELLOW);
    canvasVirtual.setCursor(270, 170 - animOffset); canvasVirtual.print("$");
  }
}

void renderizarMenuConfig() {
  canvasVirtual.fillScreen(COR_MENU_BG);
  canvasVirtual.setTextColor(WHITE); canvasVirtual.setFont(&fonts::Font4); canvasVirtual.setTextSize(0.8);
  canvasVirtual.setCursor(10, 10); canvasVirtual.print("MENU DE CONFIGURAÇÃO");
  canvasVirtual.drawFastHLine(10, 32, 300, CINZA_CLARO);

  canvasVirtual.setFont(&fonts::Font2); canvasVirtual.setTextSize(1.0);
  
  canvasVirtual.fillRect(20, 50, 280, 40, CINZA_ESCURO); canvasVirtual.drawRect(20, 50, 280, 40, WHITE);
  canvasVirtual.setCursor(35, 62); canvasVirtual.printf("MOTOR: %.1f Litros [Alterar]", telemetria.motor_litros);

  canvasVirtual.fillRect(20, 105, 280, 40, CINZA_ESCURO); canvasVirtual.drawRect(20, 105, 280, 40, WHITE);
  canvasVirtual.setCursor(35, 117); canvasVirtual.print("> VER TERMINAL DEBUG OBD2");

  canvasVirtual.fillRect(80, 185, 160, 35, RED); canvasVirtual.drawRect(80, 185, 160, 35, WHITE);
  canvasVirtual.setTextDatum(textdatum_t::top_center); canvasVirtual.drawString("VOLTAR DASH", 160, 195);
  canvasVirtual.setTextDatum(textdatum_t::top_left);
}

// CORREÇÃO: Função do Terminal de Debug reintroduzida com sucesso
void renderizarSubDebug() {
  canvasVirtual.fillScreen(BLACK);
  canvasVirtual.setTextColor(CYAN); canvasVirtual.setFont(&fonts::Font4); canvasVirtual.setTextSize(0.7);
  canvasVirtual.setCursor(10, 10); canvasVirtual.print("OBD2 TERMINAL TRAFEGO LOG");
  canvasVirtual.drawFastHLine(10, 30, 300, CINZA_ESCURO);

  canvasVirtual.setFont(&fonts::Font0); canvasVirtual.setTextSize(1.5); canvasVirtual.setTextColor(GREEN);
  int startY = 40;
  for (int i = 0; i < totalLinhasDebug; i++) {
    canvasVirtual.setCursor(10, startY + (i * 15));
    canvasVirtual.print(linhasDebug[i]);
  }

  canvasVirtual.fillRect(200, 195, 110, 35, CINZA_ESCURO); canvasVirtual.drawRect(200, 195, 110, 35, WHITE);
  canvasVirtual.setCursor(215, 205); canvasVirtual.print("VOLTAR");
}
...
void inicializarDisplay() {
  ...
  adicionarLogDebug("SYS: Inicializando canal serial OBD2...");
...
void atualizarInterfaceGrafica() {
  ...
  if (xQueueReceive(xFilaTouch, &ev, 0) == pdTRUE) {
    switch (ev) {
      ...
      case 5: modoAtual = TELA_MENU_CONFIG; break;
      default: break;
    }
  }
